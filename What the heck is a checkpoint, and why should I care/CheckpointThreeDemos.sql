...
IF EXISTS (SELECT 1 FROM sys.databases WHERE [name] = N'hrssugdemo2')
  BEGIN
    SET @SQL = 
      N'USE [master];
       ALTER DATABASE hrssugdemo2 SET SINGLE_USER WITH ROLLBACK IMMEDIATE;
       USE [master];
       DROP DATABASE hrssugdemo2;';
    EXEC (@SQL);
  END;
ELSE
  BEGIN
...
USE master;
GO
ALTER DATABASE hrssugdemo2 SET RECOVERY SIMPLE;
GO
ALTER DATABASE hrssugdemo2 SET AUTO_CREATE_STATISTICS OFF;
GO
...
EXEC hrssugdemo2..p_StressTestTable_ins
EXEC hrssugdemo2..p_StressTestTable_upd
...
*/
ALTER DATABASE hrssugdemo2 SET TARGET_RECOVERY_TIME = 15 SECONDS;
GO
ALTER DATABASE hrssugdemo2 SET TARGET_RECOVERY_TIME = 120 SECONDS;
GO
...
USE master;
GO
DROP DATABASE IF EXISTS hrssugdemo2;
GO