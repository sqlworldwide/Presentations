...
IF EXISTS (SELECT 1 FROM sys.databases WHERE [name] = N'hrssugdemo')
  BEGIN
    SET @SQL = 
      N'USE [master];
       ALTER DATABASE hrssugdemo SET SINGLE_USER WITH ROLLBACK IMMEDIATE;
       USE [master];
       DROP DATABASE hrssugdemo;';
    EXEC (@SQL);
  END;
ELSE
  BEGIN
...
USE master;
GO
ALTER DATABASE hrssugdemo SET RECOVERY SIMPLE;
GO
ALTER DATABASE hrssugdemo SET AUTO_CREATE_STATISTICS OFF;
GO

/*
Drop table if exists
Creata an empty table
...
Then run the buffer page count query again. Notice the change in number of dirty pages to clean pages.
*/
CHECKPOINT;
GO
SELECT 
...
USE master;
GO
DROP DATABASE IF EXISTS hrssugdemo;
GO