...
CREATE USER elasticJobTarget FOR LOGIN  elasticJobTarget WITH DEFAULT_SCHEMA =dbo;
GO
...
GO
CREATE DATABASE SCOPED CREDENTIAL elasticJobTargetCredential WITH IDENTITY = 'elasticJobTarget', SECRET ='Pa$$w0rd123'; 
GO
...
--View the recently created target group and target group members
SELECT * FROM jobs.target_groups WHERE target_group_name='PoolGroup';
SELECT * FROM jobs.target_group_members WHERE target_group_name='PoolGroup';
